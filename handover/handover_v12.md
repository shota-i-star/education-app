...
| 対象 | 内容 |
|------|------|
| login.html | dept_admin ログイン後に manager_dashboard.html へ遷移するよう追加 |
| manager_dashboard.html | 自部署メンバーのみ表示・昇級申請も自部署のみ |
...
### フェーズ5で完了したカテゴリ管理再設計
- `category_departments` 中間テーブルを新規作成（1カテゴリ→1部署に1:1で対応）
- `categories.name` のユニーク制約を解除（部署ごとに同名カテゴリを許可）
- カテゴリは部署ごとに完全独立したレコードとして管理
- カテゴリ複製機能：カテゴリ・項目・ステップをまるごと別部署にコピー可能
- カテゴリ・項目管理の表示を「部署→等級→カテゴリ→項目」構造に変更
- テスト管理の表示を「部署→等級→問題集→問題」構造に変更

### フェーズ5で完了したUI改善
- 部署カード・等級セクション・カテゴリカードにトグル開閉を追加（▾矢印）
- 項目追加モーダルにステップ複数同時入力欄を追加（Enterで連続追加・一括保存）
- ダッシュボードのエリア別・店舗別タブで各行クリック→メンバー一覧を展開表示

### boss → manager リネーム
| 変更前 | 変更後 |
|--------|--------|
| `boss_dashboard.html` | `manager_dashboard.html` |
| ロール値 `'boss'` | ロール値 `'manager'` |
| ロール値 `'boss_admin'` | ロール値 `'manager_admin'` |

Supabase SQL で既存ユーザーのロール値も更新済み：
```sql
UPDATE user_profiles SET role = 'manager'       WHERE role = 'boss';
UPDATE user_profiles SET role = 'manager_admin'  WHERE role = 'boss_admin';
```
...
education-app/
├── login.html              ✅ 完成
├── member_top.html         ✅ 完成（ステップ式チェック・昇級申請対応済み）
├── test_page.html          ✅ 完成（再試験機能対応）
├── manager_dashboard.html  ✅ 完成（dept_adminフィルター・エリア/店舗展開対応済み）
├── member_detail.html      ✅ 完成（コメント機能含む）
├── admin.html              ✅ 完成（dept_adminフィルター・トグル・複製機能対応済み）
├── spec_doc v12.md         📄 仕様書（最新）
└── handover_v12.md         📄 引継書（最新）
```
...
| category_departments | カテゴリ×部署の中間テーブル（1カテゴリ:1部署） |
...
| 鈴木 部長 | 00001 | suzuki1234 | manager_admin | － |
...
### dept_adminのフィルター方針
```javascript
// manager_dashboard.html / admin.html 共通のグローバル変数
...
### カテゴリ管理のポイント
```javascript
// カテゴリ追加時：部署ごとに新しいレコードを必ず INSERT（同名でも別レコード）
for (var i = 0; i < selectedDeptIds.length; i++) {
  var { data: catData } = await client.from('categories').insert({ name: name }).select().limit(1);
  await client.from('category_departments').insert({ category_id: catData[0].id, department_id: deptId });
}
...
```
添付の仕様書（spec_doc v12.md）と引継書（handover_v12.md）をもとに、
